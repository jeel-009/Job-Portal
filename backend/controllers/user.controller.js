...
export const login = async (req, res) => {
    try {
        ...
        //check password
        const isPasswordisHash = await bcrypt.compare(password, user.password)
        if (!isPasswordisHash) {
            return res.status(400).json({
        ...
        if (role !== user.role) {
            return res.status(400).json({
                message: "Dose't Exits with currnt roles",
                success: false
            })
        }
        const tokenData = {
        ...
        return res.status(200).cookie('token', token, {
            maxAge: 1 * 24 * 60 * 60 * 1000, httpOnly: true,//fornted js acces nahi kr sakta (document.cookies)
            sameSite: 'strict'//dusri website tumhari cookies nahi use kar sakti
        }).json({
...
export const updateProfile = async (req, res) => {
    try {
        ...
        const userId = req.id; //middleware se ayegi
